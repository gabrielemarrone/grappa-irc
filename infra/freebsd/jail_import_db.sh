...
#   ssh m42 sudo cp /tmp/local-grappa-prod.db /usr/local/bastille/jails/grappa-new/root/tmp/grappa_prod.db
#   sudo bastille cmd grappa-new /home/grappa/grappa/infra/freebsd/jail_import_db.sh
#
...
if [ ! -r "${SRC}" ]; then
	...
	echo "  sudo cp /tmp/grappa_prod.db /usr/local/bastille/jails/grappa-new/root/tmp/" >&2
	exit 1
fi

if [ -r "${PID_FILE}" ] && kill -0 "$(cat "${PID_FILE}")" 2>/dev/null; then
	echo "[import_db] ERROR: grappa is running (pid $(cat "${PID_FILE}")). Stop it first:" >&2
	echo "  sudo bastille cmd grappa-new service grappa stop" >&2
	exit 1
fi
...
echo "[import_db] done. Start service with: sudo bastille cmd grappa-new service grappa start"
